...
if round(sum(S.forage_type),1) ~= 1
    warning("<strong>CAUTION:</strong> Forage proportions don't add up to 100%%")
end
     

% Display error if grain% values don't add up to 100
if round(sum(S.grain_type),1) ~= 1
    warning("<strong>CAUTION:</strong> Grain proportions don't add up to 100%%")
end


% Display error if activity% values don't add up to 100
if sum(S.P_activity) ~= 1
    warning("<strong>CAUTION:</strong> Grazing practices'" + ...
            " proportions don't add up to 100%%")
end


% Display error birth% values don't add up to 100
if round(sum(S.P_birth),1) ~= 1
    warning("<strong>CAUTION:</strong> Pregnancy proportions don't add up to 100%%")
end


% Display error if manure management% values don't add up to 100
if sum(S.MS) ~= 1
    warning("<strong>CAUTION:</strong> Manure management doesn't add up to 100%%")
end


% Display error if fertilizer type% values don't add up to 100
if round(sum(S.fertilizer_type),1) ~= 1
    warning("<strong>CAUTION:</strong> Fertilizer proportions don't add up to 100%%")
end

% Display error if enterprise is not defined
if exist('enterprise', 'var') ~= 1
    warning("<strong>CAUTION:</strong> Enterprise value must be 1, 2, or 3")
end
